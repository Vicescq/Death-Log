...
export default function GameProfiles({ gameID }: { gameID: string }) {
    ...
    function handleAdd(inputText: string, autoDate: boolean = true) {
        ...
        ContextManager.updateActionHistory(history, setHistory, new Action("add", [node]), new Action("update", [tree.get(gameID!)!]));
    }
    ...
    function handleCompletedStatus(profile: Profile, newStatus: boolean) {
        ...
        if (bool) {
            ...
            ContextManager.updateActionHistory(history, setHistory, new Action("update", [profile]));
        }
    }
